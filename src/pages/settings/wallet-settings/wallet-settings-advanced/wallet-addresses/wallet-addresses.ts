  ...
  ionViewWillEnter() {
    ...
    this.walletProvider.getLowUtxos(this.wallet).then((resp) => {
      ...
      if (resp && resp.allUtxos && resp.allUtxos.length) {
        ...
        this.lowUtxosSum = this.txFormatProvider.formatAmountStr('BSTK', _.sumBy(resp.lowUtxos || 0, 'satoshis'));
        this.allUtxosSum = this.txFormatProvider.formatAmountStr('BSTK', allSum);
        this.minFee = this.txFormatProvider.formatAmountStr('BSTK', resp.minFee || 0);
